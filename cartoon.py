...
def sketch_image(img):
    ...
    # Apply median filter
    #img_gray = cv2.GaussianBlur(img_gray, (0,0), 1)

    # Detect edges using cv2.Laplacian()
    edges = cv2.Laplacian(img_gray, cv2.CV_8U, ksize=5)

    # Threshold the edges image:
    #ret, thresholded = cv2.threshold(edges, 50, 255, cv2.THRESH_BINARY)
...
cv2.imshow('im', cv2.cvtColor(custom_sketch_image, cv2.COLOR_GRAY2BGR))
cv2.waitKey(0)
